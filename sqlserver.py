...
class three():
    def query_1(connect, name):  # 按船只名称查询对应的各类证书：营运证书、检验证书、安检证书、国籍配员证书的基本信息
        ...
        sql_1 = "select COLUMN_NAME from information_schema.COLUMNS where table_name = '港澳航线船舶营运证' "

        cursor.execute(sql_1)
        head_1 = check(cursor)
        query_sql_1 = "select *from 港澳航线船舶营运证 where 船名= %s"
        cursor.execute(query_sql_1, name)
        query_data_1 = check(cursor)

        sql_2 = "select COLUMN_NAME from information_schema.COLUMNS where table_name = '船舶最低安全配员证书' "
        cursor.execute(sql_2)
        head_2 = check(cursor)

        query_sql_2 = "select *from 船舶最低安全配员证书 where 船名= %s"
        cursor.execute(query_sql_2, name)
        query_data_2 = check(cursor)

        sql_3 = "select COLUMN_NAME from information_schema.COLUMNS where table_name = '船舶国籍证书' "
        cursor.execute(sql_3)
        head_3 = check(cursor)

        query_sql_3 = "select *from 船舶国籍证书  where 船名= %s"
        cursor.execute(query_sql_3, name)
        query_data_3 = check(cursor)

        sql_4 = "select COLUMN_NAME from information_schema.COLUMNS where table_name = '货船适航证书' "
        cursor.execute(sql_4)
        head_4 = check(cursor)
        query_sql_4 = "select *from 货船适航证书 where 船名= %s"
        cursor.execute(query_sql_4, name)
        query_data_4 = check(cursor)

        return head_1, head_2, head_3, head_4, query_data_1, query_data_2, query_data_3, query_data_4
...
class four():
    ...
    def add_2(connect,
              val):  # 录入(`船名`,`船舶种类`,`船体材料`,`主机种类`,`船籍港`,`造船地点`,`登记号码`,`建成日期`,`总长`,`型宽`,`型深`,`总吨`,`净吨`,`功率`,`载重`)
        cursor = connect.cursor()
...
class five():
    def add(connect,
            val):  # 录入新增加的船只的营运证书的基本信息到数据库中并关联起船只名称和船检登记号：包括营运证编号、船舶所有人、船舶登记号、经营人许可证号码、管理人许可证号码、发证机关、营运证使用有效期至、发证日期
        cursor = connect.cursor()
